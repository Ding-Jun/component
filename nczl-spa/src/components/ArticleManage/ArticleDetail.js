...
class ArticleDetail extends React.Component{
  ...
  handleUploadFile(e){
    ...
    if(!file){console.log("未选择图片")}
    else{
      var fileForm=new FormData();
      fileForm.append("file",file);
      console.log("debug target",file)
      $.ajax({
        type: 'GET',
        url: `/nczl-web/rs/article/list?curPage=1&pageSize=5`,
        dataType: 'jsonp',
        jsonp:"callback",
        data:fileForm,
        processData: false,  // 告诉jQuery不要去处理发送的数据
        contentType:false,
        jsonpCallback: function (rm) {
          /*if (rm.code == 1) {
            console.log('debug', rm.result);
            this.setState({
            })
  ...
  handleTest(e){
    ...
    $("#img-container").html('<img src="../../images/head1.jpg" style="width: 100%;height:100%">');
  ...
  renderForm(){
    ...
    var columnSelect=this.state.columns.map((column)=>{return <option key={column.id} value={column.id}>{column.columnName}</option>})
    columnSelect=<select value={this.state.id == 'new'?1:article.columnId} name="columnId" onChange={this.handleFieldChange.bind(this)}>{columnSelect}</select>
    ...
    return (
      <Form ref="form" id="form" onSubmit={this.handleSubmit.bind(this)}>
        <FromItem label="标题：" required={!readOnly}>
          {readOnly?article.title:<Input size="60" value={article.title} name="title" onChange={this.handleFieldChange.bind(this)}/>}
        </FromItem>
        <FromItem label="作者&身份：" required={!readOnly}>
          {readOnly?article.author:<Input size="20" value={article.author} name="author" onChange={this.handleFieldChange.bind(this)}/>}
        </FromItem>
        <FromItem label="所属分类：" required={!readOnly}>
          {readOnly?article.columnName:columnSelect}
        </FromItem>
        <FromItem label="状态：" >
          {readOnly?status:<select value={article.status} name="status" onChange={this.handleFieldChange.bind(this)}>
            <option value={1}>马上发布</option>
            <option value={0}>停止</option>
          </select>}
        </FromItem>
        <FromItem label="置顶：" >
          {readOnly ? (article.stick?'是':'否') : <select  value={article.stick} name="stick" onChange={this.handleFieldChange.bind(this)}>
        ...
        <FromItem label="置顶标题图：" required={!readOnly} help="上传尺寸：640px*350px，图片格式仅限为：jpg、gif、png，文件大小为500KB以内。">
          <div><img id="avatar" src={article.stickImg}/></div>
          {readOnly?null:<span><Input ref="stickImgSrc" id="stickImgSrc" type="file" size="40" onChange={this.handleUploadFile.bind(this)}/></span>}
        </FromItem>:null}
        <FromItem label="文章标题缩略图：" required={!readOnly} help="上传尺寸：200px*150px，图片格式仅限为：jpg、gif、png，文件大小为200KB以内。">
          <div><img src={article.titleImg}/></div>
          {readOnly?null:<span><Input ref="titleImgSrc" id="titleImgSrc" type="file" size="40" onChange={this.handleUploadFile.bind(this)}/></span>}
        </FromItem>
        {readOnlyContents}
        <FromItem label="摘要：" required={!readOnly}>
          {readOnly?article.summary:<span>
                  <Input type="textarea" cols="90" rows="5" value={article.summary} name="summary" onChange={this.handleFieldChange.bind(this)}/>
                <br />
                  字数上限：50</span>}

        </FromItem>
        <FromItem label="正文：" required={!readOnly}>
          {readOnly?article.contents:<Input type="textarea" cols="90" rows="5" value={article.contents} name="contents" onChange={this.handleFieldChange.bind(this)}/>}
